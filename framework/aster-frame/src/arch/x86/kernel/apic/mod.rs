...
cpu_local! {
    static APIC_INSTANCE: OnceCell<RefCell<Box<dyn Apic + 'static>>> = OnceCell::new();
}

pub fn is_init() -> bool {
    APIC_INSTANCE.with(|a| a.get().is_some())
}

pub fn with_apic<T>(f: impl FnOnce(&mut (dyn Apic + 'static)) -> T) -> T {
    APIC_INSTANCE.with(|a| f(&mut **a.get().unwrap().borrow_mut()))
}

pub fn try_with_apic<T>(f: impl FnOnce(&mut (dyn Apic + 'static)) -> T) -> Option<T> {
    APIC_INSTANCE.with(|a| a.get().map(|a| f(&mut **a.borrow_mut())))
}
...
pub fn init() -> Result<(), ApicInitError> {
    ...
    if let Some(mut x2apic) = x2apic::X2Apic::new() {
        ...
        APIC_INSTANCE.with(|a| drop(a.get_or_init(|| RefCell::new(Box::new(x2apic)))));
        Ok(())
        ...
        APIC_INSTANCE.with(|a| drop(a.get_or_init(|| RefCell::new(Box::new(xapic)))));
        Ok(())
    } else {
